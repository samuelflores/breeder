...
#include <sstream>
	...
	std::string MysqlConnection::runJobString(std::string mySingleMutantID,std::string myWorkingDirectory,  std::string mySingleMutantDirectory,  bool computeIndividualWildType ) {
			...
			struct stat st;
                ...
                std::cout<<__FILE__<<":"<<__LINE__<<std::endl;
                runJobStringStream<<"# "<<__FILE__<<":"<<__LINE__<<std::endl;
                runJobStringStream<<               myFoldXScript<<" -mainDirectory "<< myWorkingDirectory  <<" -mutationString  " <<  mySingleMutantID <<" -jobName  "<< myJobID<<" -computeIndividualWildType  "<< computeIndividualWildType<< " -foldxExecutable  "<<myFoldXExecutable  << " -SQLExecuteString  "<< mySQLExecutableAndParameters<<" -complexString  "<<myChainsInMutatedSubunit<<" -myLastStage "<< myLastStage  << " -myPdbId "<< getPdbId() <<" -temperature  "<< getTemperatureInK() << " &> "<<mySingleMutantDirectory<<"/run-foldx.out; "<<std::endl;//  ; " <<std::endl;

	...
	void Chromosome::prepareSingleMutant( DBManager & mysqlConnection , std::string singleMutantDirectory, bool computeIndividualWildType) { //, std::string jobName, std::string workingDirectory, std::string singleMutantDirectory  ) {
		...
		*/
		...
		checkOrCreateDirectory(singleMutantDirectory);
			    ...
			    startJobString = std::string("insert into results (jobName, pdbId, batch_directory, sequence,mutationString,complexString,pdbNumberedMutationString, status,numMutations ) VALUES (\"")    +  mysqlConnection.getJobName()  + "\", \"" +   mysqlConnection.getPdbId()  +  "\",   \""    +  workingDirectory  + std::string("\", \"")  + getSequences() + std::string("\" , \"") + getMutationString() + std::string("\", \"") + mysqlConnection.getChainsInMutatedSubunit() + std::string("\" , \"")  + getPdbNumberedMutationString(mysqlConnection) + std::string("\" , \"submitted\", ") + std::to_string(numMutations()) + std::string(");");
                        } else {
			    startJobString = std::string("insert into results (jobName, pdbId, batch_directory, sequence,mutationString,complexString,                           status,numMutations ) VALUES (\"")    +  mysqlConnection.getJobName()  + "\", \"" +   mysqlConnection.getPdbId()  +  "\",   \""    +  workingDirectory  + std::string("\", \"")  + getSequences() + std::string("\" , \"") + getMutationString() + std::string("\", \"") + mysqlConnection.getChainsInMutatedSubunit()                                                                           + std::string("\" , \"submitted\", ") + std::to_string(numMutations()) + std::string(");");
                        }

...
$runFoldXCommand = "cd $singleMutantDirectory; $foldxExecutable --command=RepairPDB --pdb=$cyxToCysPdb --repair_Interface=ONLY --temperature=$temperature  --water=-CRYSTAL --pdbHydrogens=false  > $singleMutantDirectory/foldx.$mutationString.out ;  $foldxExecutable --command=AnalyseComplex --pdb=$cyxToCysRepairPdb --analyseComplexChains=$chainsInMutatedSubunit --temperature=$temperature  >> $singleMutantDirectory/foldx.$mutationString.out ";

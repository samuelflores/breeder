...
$runFoldXCommand = "cd $singleMutantDirectory; $foldxExecutable --command=RepairPDB --pdb=$cyxToCysPdb --repair_Interface=ALL > $singleMutantDirectory/foldx.$mutationString.out ;   $foldxExecutable --command=AnalyseComplex --pdb=$cyxToCysRepairPdb    --analyseComplexChains=$chainsInMutatedSubunit >> $singleMutantDirectory/foldx.$mutationString.out ";

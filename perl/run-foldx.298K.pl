...
$runFoldXCommand = "cd $singleMutantDirectory; $foldxExecutable --command=RepairPDB --pdb=$cyxToCysPdb --repair_Interface=ALL > $singleMutantDirectory/foldx.$mutationString.out ; $foldxExecutable --command=Optimize --pdb=$cyxToCysRepairPdb >> $singleMutantDirectory/foldx.$mutationString.out ; $foldxExecutable --command=AnalyseComplex --pdb=$cyxToCysOptimizedPdb --analyseComplexChains=$chainsInMutatedSubunit --temperature=298 >> $singleMutantDirectory/foldx.$mutationString.out ";
print "check 1\n";
...
if ($computeIndividualWildType){
# Turns out on akka for some reason the &> redirect does not work. Leads instead to output being dumped to what looks like stdout (or maybe stderr). Using just > redirect instead.
$mysqlString = "echo \"update results set foldx_energy_wild_type = $foldx_energy, status = \\\"WT completed\\\" , date = \\\"$year-$month-$day\\\"  where jobName = \\\"$jobName\\\" and mutationString = \\\"$correspondingMutant\\\";\" | $SQLExecuteString  >  $singleMutantDirectory/$mutationString.mysql ";
...
# Turns out on akka for some reason the &> redirect does not work. Leads instead to output being dumped to what looks like stdout (or maybe stderr). Using just > redirect instead.
$mysqlString = "echo \"update results set foldx_energy = $foldx_energy, status = \\\"completed\\\" , date = \\\"$year-$month-$day\\\"  where jobName = \\\"$jobName\\\" and mutationString = \\\"$mutationString\\\";\" | $SQLExecuteString  &>  $singleMutantDirectory/$mutationString.mysql ";
}
#$fileLocked = 1;
#while ($fileLocked) {
